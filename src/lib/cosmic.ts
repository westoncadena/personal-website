...
import { Post, GlobalData, Author } from '@lib/types';

// Check for environment variables and throw an error if they are not defined
const bucketSlug = process.env.COSMIC_BUCKET_SLUG;
const readKey = process.env.COSMIC_READ_KEY;

if (!bucketSlug || !readKey) {
    throw new Error('COSMIC_BUCKET_SLUG and COSMIC_READ_KEY must be defined in the environment variables.');
}
...
export async function getGlobalData(): Promise<GlobalData> {
    // Get global data
    try {
        const data: { object: GlobalData } = await Promise.resolve(
            cosmic.objects
                .findOne({
                    type: 'globals',
                    slug: 'header',
...
export async function getAllPosts(): Promise<Post[]> {
    try {
        // Get all posts
        const data: { objects: Post[] } = await Promise.resolve(
            cosmic.objects
                .find({
                    type: 'posts',
                })
...
export async function getPost(slug: string): Promise<Post> {
    try {
        // Get post
        const data: { object: Post } = await Promise.resolve(
            cosmic.objects
                .findOne({
                    type: 'posts',
                    slug,
...
export async function getAuthor(slug: string): Promise<Author> {
    try {
        const data: { object: Author } = await Promise.resolve(
            cosmic.objects
                .findOne({
                    type: 'authors',
                    slug,
